...
constexpr size_t MAX_DATA_FILE_SIZE = 500 * 500;
...
static double to_value(img::pixel_t const& pix)
{
	auto const rgb = img::to_rgb(pix);

	return to_value(rgb.r);
}


static std::string make_file_name(unsigned index, size_t index_length)
{
	char idx_str[10];
	...
	oss << std::put_time(std::localtime(&result), "%Y-%m-%d_%H-%M-%S");
...
namespace convert_data
{
	...
	{
		const auto max_height = MAX_DATA_FILE_SIZE / DATA_IMAGE_WIDTH;
		...
		{
			const auto name = make_file_name(idx++, idx_len);
	...
	{
		src_data_t data;
		data.reserve(row_view.width() * row_view.height());

		gil::for_each_pixel(row_view, [&](auto const& p) { data.push_back(to_value(p)); });
